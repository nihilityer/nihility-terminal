...
use crate::core::instruct_manager::{ENCODE_SIZE_FIELD, InstructManager, PointPayload};
...
impl InstructManager for GrpcQdrant {
        ...
        Self: Sized,
    {
        ...
        let qdrant_client =
            QdrantClientConfig::from_url(config.get(QDRANT_GRPC_ADDR_FIELD).unwrap()).build().map_err(|err| eyre!(Box::new(err)))?;
        let encode_size = config.get(ENCODE_SIZE_FIELD).unwrap().parse::<u64>()?;

        let mut collection_created = false;
        let collections = qdrant_client.list_collections().await.map_err(|err| eyre!(Box::new(err)))?;
        ...
        if !collection_created {
            qdrant_client
                ...
                .await.map_err(|err| eyre!(Box::new(err)))?;
        }
    ...
    async fn search(&self, encode: Vec<f32>) -> Result<String> {
        let mut search_result = Vec::<ScoredPoint>::new();
        {
            ...
            let search_resp = self.qdrant_client.search_points(&search_req).await.map_err(|err| eyre!(Box::new(err)))?;
        ...
        return Err(eyre!("Cannot Search Match Submodule By This Instruct"));
    }
